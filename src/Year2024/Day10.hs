module Year2024.Day10 (solve) where
...
-}
part1 :: String -> Int
part1 s = sum . map (`trailScores` grid) . positionsOf '0' $ grid
  where
    grid = Grid2D . lines $ s
...
trailScores p g = length $ foldl' getNext (S.singleton p) ("123456789" :: String)
  where
    ...
    getNext ps' v =
        S.fromList
            . concatMap (filter (\p' -> equals v p' g) . directNeighbors)
            $ ps'
...
A trailhead's rating is the number of distinct hiking trails which begin at that
...
-}
part2 :: String -> Int
part2 s = sum . map (`trailScores'` grid) . positionsOf '0' $ grid
  where
    grid = Grid2D . lines $ s
...
solve :: IO ()
solve = do
    content <- readFile "./src/Year2024/data/day10.txt"
