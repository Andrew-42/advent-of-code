...
leftZ _ = error "The list must be infinite."

rightZ :: Zipper a -> Zipper a
rightZ (Zipper lz x (r : rz)) = Zipper (x : lz) r rz
rightZ _ = error "The list must be infinite."
...
getGrid gb g = map (takeN nCols) rows
  where
    (Grid startG) = moveTo (gb.leftX, gb.topY) g
    nCols = abs (gb.rightX - gb.leftX)
    nRow = abs (gb.topY - gb.bottomY)
